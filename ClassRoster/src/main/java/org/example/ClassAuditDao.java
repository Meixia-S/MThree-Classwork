...
public class ClassAuditDao {
  private static final String AUDIT_FILE_PATH = "./StudentRosterAudit.txt";
  ...
  public void addStudentToAudit(Student student) {
    printWriter.println(student.getId()+"::"+student.getFirstName()+"::"+student.getLastName()); // Add a new line for clarity
    printWriter.flush(); // Ensure the data is written
  }

  // Method to remove a student from the audit file
  public void removeStudentToAudit(String lastName) throws IOException {
    List<String> allStudents = new ArrayList<>();

    // Read all lines and store in a list except the line to be removed
    while (scanner.hasNextLine()) {
      String currentLine = scanner.nextLine();
      if (!currentLine.contains(lastName)) {
